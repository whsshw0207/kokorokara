...
public class CMC_6 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out rayHit, 100.0f))
        {
            ...
            if (rayHit.collider.CompareTag("C"))
            {
                GageTimer += 1.0f / 2.0f * Time.deltaTime;
                if (GageTimer >= 1)
                {
            ...
            if (rayHit.collider.CompareTag("Link"))
            {
                GageTimer += 1.0f / 2.0f * Time.deltaTime;
                if (GageTimer >= 1)
                {
                    scanObject = rayHit.collider.gameObject;
                    Application.OpenURL("https://forms.gle/dJBPoMMNbSQnrRDg8");
                    GageTimer = 0;
